...
df\_a = pd.read\_csv("A.csv")

df\_b = pd.read\_csv("B.csv")



\# Hacemos el merge entre df\_a y df\_b usando la columna 'Código'

df\_merged = pd.merge(df\_a, df\_b, on="Código", how="left")



\# Opcional: eliminar la columna 'Código' si ya no la necesitas

df\_merged.drop(columns=\["Código"], inplace=True)



\# Renombrar la columna 'Nombre' como 'Código' si quieres mantener el mismo nombre

df\_merged.rename(columns={"Nombre": "Código"}, inplace=True)
...
*Esta pregunta me permitió entender porqué me aparecían columnas extras al final, y reorganicé mi método, llevándolo a reemplazar las columnas auxiliares con las columnas originales y luego borrar las auxiliares.*



https://www.geeksforgeeks.org/pandas/copying-csv-data-into-csv-files-using-python/
